...
class WaybarCrypto(object):
    ...
    def __parse_config_path(self, config_path: str):
        ...
        try:
            ...
            # currency_symbol = config["general"].get("currency_symbol","")

            # Get a list of the chosen display options
            display_options = config["general"]["display"].split(",")
            tooltip_options = config["general"]["tooltip"].split(",")
    ...
    def get_json(self) -> str:
        ...
        tooltip_options = self.config["tooltip_options"]
        ...
        for coin_name, coin_obj in self.config["coins"].items():
            ...
            if "price" in display_options or not display_options:
                current_price = f'{round(Decimal(pair_info["price"]), price_precision):,}'
                output += f"{current_price} "
            ...
            if "volume24h" in display_options:
                ...
                output += f"24hV:{percentage_change:+} "
            ...
            if "change1h" in tooltip_options:
                percentage_change = round(
                    Decimal(pair_info["percent_change_1h"]), change_precision
                )
                tooltip += f"{percentage_change:+}  "
            ...
            if "change24h" in tooltip_options:
                percentage_change = round(
                    Decimal(pair_info["percent_change_24h"]), change_precision
                )
                tooltip += f"{percentage_change:+}  "
            ...
            output_obj["text"] += f"{output}  "
            output_obj["tooltip"] += f"{tooltip}  \n"

        output_obj["text"] = output_obj["text"][:-3]
